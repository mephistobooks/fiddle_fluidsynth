...
fs.settings.each_setting{|data, name, type|
  begin
    ...
    #
    # mname = FiddleFluidSynth::Interface::Settings.item_to_meth_name(name)
    ...
    if fs.setting_type_is_str?(type) && 
      ...
      puts "    {each_option_of} #{name} is string and optionlist:"
  ...
  #
    ...
    $stdout.puts "#{ex.message}:#{ex.class}"
    ex.backtrace.each do |_msg|
      $stderr.puts "  |#{_msg}"
      $stdout.puts "  |#{_msg}"
    end

    raise  # actually this is meaningless. we cannot raise from here.
  end
}
puts
